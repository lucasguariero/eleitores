...
=======
  ...
  return <>
    ...
    <div className="table-card"><table><thead><tr><th>Pessoa</th><th>Vínculo</th><th>Cargo no grupo</th><th>Área</th><th>Perfil de acesso</th><th>Situação</th></tr></thead><tbody>{team.map(pessoa => <tr key={pessoa.name}>
>>>>>>> 272acb0232a7b3ff0de23644e36d1f48ca436cbe
...
=======
    <PageHeader eyebrow="Preferências pessoais" title="Minha conta" description="Só o que afeta você neste dispositivo e nesta conta. O que vale para todo o ambiente — identidade, vocabulários, planos e retenção — mora em Administração → Parametrizações." actions={salvo ? <span className="confirmacao"><Check size={15} /> Alterações salvas</span> : undefined} />
>>>>>>> 272acb0232a7b3ff0de23644e36d1f48ca436cbe
...
<<<<<<< HEAD
  ...
  const parcial = municipios.filter(m => m.completude >= 40 && m.completude < 80).length
  ...
  return <>
    ...
    <div className="metrics-grid five">
      ...
      <Metric label="Dossiê parcial" value={String(parcial)} delta="entre 40% e 80%" icon={SlidersHorizontal} tone="amber" />
...
<<<<<<< HEAD
      ...
      <AccordionBlock title="Resultados das eleições" icon={Vote}>
        <div className="resultados-ano">{resultados.map(r => { const total = r.candidatos.reduce((s, x) => s + x.votos, 0); return <div key={r.ano} className="resultado-ano"><h4>{r.ano} <small>{total.toLocaleString('pt-BR')} votos</small></h4>{r.candidatos.map((c, i) => <div key={i} className="resultado-candidato"><span className="candidato-info"><strong>{c.nome}</strong><small>{c.partido}</small></span><span className="candidato-votos"><b>{c.votos.toLocaleString('pt-BR')} votos</b><small>{Math.round(c.votos / total * 100)}%</small></span><div className="barra-votos"><i style={{ width: `${c.votos / r.candidatos[0].votos * 100}%` }} className={c.eleito ? 'vencedor' : ''} /></div>{c.eleito && <Pill tone="green">Eleito</Pill>}</div>)}</div> })}</div>
      </AccordionBlock>
=======
...
function MesorregiaoProfile({ id, navigate }: { id: string; navigate: Navigate }) {
  ...
  const parcial = municipiosDaDivisao.filter(m => m.completude >= 40 && m.completude < 80).length
...
=======
    <PageHeader eyebrow="Relacionamento" title="Agenda" description="Viagens, visitas, reuniões e entrevistas num só lugar. Reunião é um tipo de compromisso, não um módulo à parte — e qualquer tipo pode gerar encaminhamento com dono e prazo." actions={<button className="primary-button"><Plus size={16} /> Novo compromisso</button>} />
>>>>>>> 272acb0232a7b3ff0de23644e36d1f48ca436cbe
